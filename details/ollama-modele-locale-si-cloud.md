...
Plus modele `:cloud` (proxy, nu local — verificate pe ollama.com/search?c=cloud): `glm-5.2`, `kimi-k2.7-code`, `minimax-m3`, `deepseek-v4-pro`, `gemma4`, `gpt-oss:120b-cloud` etc. (local rămâne `gpt-oss:120b`, fără sufix)
...
- **Gemini:** există totuși `gemma4:cloud` — modelul Google Gemini 3 Flash Preview servit prin Ollama Cloud (pentru hosting și tratamentul datelor se aplică politica Ollama Cloud). Prima fisură în regula „doar open-weight".
...
Așadar singura familie Google pe care o poți rula **local** prin Ollama este **Gemma** (open-weight); Gemini de frontieră există doar ca proxy `gemma4:cloud`. Vrei experiența completă Gemini? Folosești API-ul Google sau **Antigravity IDE** — platforma agentică Google (IDE + CLI + SDK): descarcă de la [antigravity.google](https://antigravity.google/) (Linux/macOS/Windows).
