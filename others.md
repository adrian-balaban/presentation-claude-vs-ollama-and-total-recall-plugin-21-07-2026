...
| `src/tools/recall.ts` | Poarta "hybrid" care rulează TF-IDF + vector search în paralel și le fuzionează. |
...
- **Cod sursă:** `../total-recall/plugins/total-recall/src/` (`embeddings.ts`, `vectorStore.ts`, `rrf.ts`, `tfidf.ts`, `ebbinghaus.ts`, `tools/recall.ts`, `server.ts`)
